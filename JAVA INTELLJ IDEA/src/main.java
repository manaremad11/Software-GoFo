    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (mainChoice == 1) {
                ...
                if(!user.checkPasswordAuthorization(password)) {
                    System.out.println("Password is not available , try again, please");
                    continue;
                }
                        ...
                        if (ownerChoice == 1) {
                            ...
                            if (playerSlotChoice <= timeslotArraySize && playerSlotChoice > 0) {
                                ...
                                System.out.println("Booking request has been sent to the playground owner... \n");
                                 int Size = user.playgroundOwners.get(index).playgrounds.get(PlaygroundIndex).Timeslot.size();


                                 for (int i = 0 ; i < Size ; i++) { //looping through the timeslots

                                     int timeslotNumber = user.playgroundOwners.get(index).playgrounds.get(PlaygroundIndex).Timeslot.get(i).charAt(0) - '0';
                                     timeslotNumber--; //decreasing the time slot number after deleting the time slot
                                     char number = (char) (timeslotNumber + '0'); //converting the timeslot  into char
                                     String str = user.playgroundOwners.get(index).playgrounds.get(PlaygroundIndex).Timeslot.get(i); //putting the string in a variable
                                     str = str.substring(0, 0) + number
                                             + str.substring(0 + 1); //putting the character in at the begining of the string variable
                                     user.playgroundOwners.get(index).playgrounds.get(PlaygroundIndex).Timeslot.set(i, str); //putting the string variable back in the arraylist
